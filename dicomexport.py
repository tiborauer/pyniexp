    ...
    def receive_scan(self,hdr=None,img=None):
        ...
        if not(self.header.is_ready_for_parsing):
            if len(self.watch_dir): 
                ...
                self.header = Header(pydicom.read_file(dcm_file))
            else:
...
hdr_ini = pickle.load(open(r'D:\Private\OneDrive - Royal Holloway University of London\Project\OpenNFT\DICOMExport\hdr_initial.pkl','rb'))
hdr = pickle.load(open(r'D:\Private\OneDrive - Royal Holloway University of London\Project\OpenNFT\DICOMExport\hdr_img0.pkl','rb'))[0]
rtd = TcpDicom()
data = {
    'watch':r'D:\Private\OneDrive - Royal Holloway University of London\Project\OpenNFT\DICOMExport',
    'LastName':'19840224SLMC',
    'ID': 'RH'
}
